...
<div class="content" :style='{"minHeight":"100vh","width":"100%","padding":"30px","background":"url(http://codegen.caihongy.cn/20240320/095a0a9d9da74a8fa6c854b2144c65aa.png) no-repeat center top / cover","height":"auto"}'>
	...
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":" 1px solid rgba(167, 180, 201,.3)    ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('minsuyuding','首页总数')">
			...
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex","order":"1"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","fontWeight":"bold","height":"24px"}'>{{minsuyudingCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>民宿预订总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgba(167, 180, 201,.3)  ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('pingjiaxinxi','首页总数')">
			<div :style='{"alignItems":"center","background":"#00e682","display":"flex","width":"80px","justifyContent":"center","height":"80px","order":"2"}'>
				<span class="icon iconfont icon-xihuan" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#374254","fontWeight":"bold","height":"24px"}'>{{pingjiaxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>评价信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type5" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="minsuyudingChart1" class="echarts1" v-if="isAuth('minsuyuding','首页统计')"></div>
		<div id="minsuyudingChart2" class="echarts2" v-if="isAuth('minsuyuding','首页统计')"></div>
		<div id="minsuyudingChart3" class="echarts3" v-if="isAuth('minsuyuding','首页统计')"></div>
		<div id="minsuyudingChart4" class="echarts4" v-if="isAuth('minsuyuding','首页统计')"></div>
		<div id="pingjiaxinxiChart1" class="echarts5" v-if="isAuth('pingjiaxinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getminsuyudingCount();
		this.minsuyudingChat1();
		this.minsuyudingChat2();
		this.minsuyudingChat3();
		this.minsuyudingChat4();
	...
	methods:{
		...
		getminsuyudingCount() {
			this.$http({
				url: `minsuyuding/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.minsuyudingCount = data.data
				}
			})
		},
		minsuyudingChat1() {
        ...
        var minsuyudingChart1 = echarts.init(document.getElementById("minsuyudingChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                minsuyudingChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    minsuyudingChart1.resize();
                };
            }
        });
      })
    ...
    minsuyudingChat2() {
      this.$nextTick(()=>{
        ...
        var minsuyudingChart2 = echarts.init(document.getElementById("minsuyudingChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                minsuyudingChart2.setOption(option);
				
				// 动画效果
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    minsuyudingChart2.resize();
                };
            }
        });
      })
    ...
    minsuyudingChat3() {
      this.$nextTick(()=>{
        ...
        var minsuyudingChart3 = echarts.init(document.getElementById("minsuyudingChart3"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                minsuyudingChart3.setOption(option);
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, minsuyudingChart3)
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    minsuyudingChart3.resize();
                };
            }
        });
      })
    ...
    minsuyudingChat4() {
      this.$nextTick(()=>{
        ...
        var minsuyudingChart4 = echarts.init(document.getElementById("minsuyudingChart4"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                minsuyudingChart4.setOption(option);
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, minsuyudingChart4)

                  //根据窗口的大小变动图表
                window.onresize = function() {
                    minsuyudingChart4.resize();
                };
            }
        });
      })
        ...
        this.$http({
            url: "pingjiaxinxi/group/xingjipingjia",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
