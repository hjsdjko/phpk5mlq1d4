<template>
	<div class="main-content" :style='{"padding":"30px","margin":"0"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#374254","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">民宿名称</label>
						<el-input v-model="searchForm.minsumingcheng" placeholder="民宿名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#374254","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">民宿类型</label>
						<el-input v-model="searchForm.minsuleixing" placeholder="民宿类型" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('minsuyuding','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('minsuyuding','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu15" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('minsuyuding','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('minsuyuding','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						批量支付
					</el-button>

					<el-button class="btn18" v-if="isAuth('minsuyuding','民宿类型')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						民宿类型
					</el-button>
					<el-button class="btn18" v-if="isAuth('minsuyuding','民宿营收')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						民宿营收
					</el-button>
					<el-button class="btn18" v-if="isAuth('minsuyuding','预订天数')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						预订天数
					</el-button>
					<el-button class="btn18" v-if="isAuth('minsuyuding','月营收')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						月营收
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='false' prop="minsutupian" width="200" label="民宿图片">
						<template slot-scope="scope">
							<div v-if="scope.row.minsutupian">
								<img v-if="scope.row.minsutupian.substring(0,4)=='http'" :src="scope.row.minsutupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.minsutupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('minsuyuding','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('minsuyuding','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('minsuyuding','评价')" @click="pingjiaxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已评价')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								评价
							</el-button>
							<el-button class="edit" v-if=" isAuth('minsuyuding','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('minsuyuding','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="minsuleixingChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yingfujineChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yudingtianshuChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yingfujineChart4" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			pingjiaxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','minsuyuding');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var minsuleixingChart1 = echarts.init(document.getElementById("minsuleixingChart1"),'macarons');
        this.$http({
            url: "minsuyuding/group/minsuleixing",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].minsuleixing);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].minsuleixing
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                minsuleixingChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    minsuleixingChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        // minsumingcheng minsumingcheng
        //  yingfujine

        var yingfujineChart2 = echarts.init(document.getElementById("yingfujineChart2"),'macarons');
        this.$http({
            url: `minsuyuding/value/minsumingcheng/yingfujine`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yingfujineChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yingfujineChart2.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var yudingtianshuChart3 = echarts.init(document.getElementById("yudingtianshuChart3"),'macarons');
        this.$http({
            url: `minsuyuding/value/minsumingcheng/yudingtianshu`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yudingtianshuChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yudingtianshuChart3.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog4() {
      ...
      this.$nextTick(()=>{
        ...
        var yingfujineChart4 = echarts.init(document.getElementById("yingfujineChart4"),'macarons');
        this.$http({
            url: `minsuyuding/value/yudingshijian/yingfujine/月`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yingfujineChart4.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yingfujineChart4.resize();
                };
            }
        });
      })
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "minsuyuding/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
