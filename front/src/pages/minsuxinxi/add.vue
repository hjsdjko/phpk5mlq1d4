...
<div :style='{"width":"100%","padding":"80px 10%","margin":"10px auto","position":"relative","background":"#F6F6F6"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="民宿类型" prop="minsuleixing">
            <el-select v-model="ruleForm.minsuleixing" placeholder="请选择民宿类型" :disabled=" false  ||ro.minsuleixing" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="民宿图片" v-if="type!='cross' || (type=='cross' && !ro.minsutupian)" prop="minsutupian">
            <file-upload
            tip="点击上传民宿图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="民宿图片" prop="minsutupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.minsutupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="民宿地址" prop="minsudizhi">
            <el-input v-model="ruleForm.minsudizhi" 
                placeholder="民宿地址" clearable :disabled=" false  ||ro.minsudizhi"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="民宿介绍" prop="minsujieshao">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='minsutupian'){
              this.ruleForm.minsutupian = obj[o].split(",")[0];
        ...
        this.$http.get('option/minsuleixing/minsuleixing', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`minsuxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('minsuxinxi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`minsuxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      minsutupianUploadChange(fileUrls) {
          this.ruleForm.minsutupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
