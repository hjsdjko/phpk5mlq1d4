...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>民宿名称：</div>
        <el-input v-model="formSearch.minsumingcheng" placeholder="民宿名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('minsuxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"10px","background":"#FFDFA6","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/minsuxinxiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"padding":"20px 10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"1px solid #5FE3EB","width":"200px","padding":"10px","overflow":"hidden","borderRadius":"10px","height":"250px"}' class="img-box">
				<img @click.stop="imgPreView(item.minsutupian)" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-if="item.minsutupian && item.minsutupian.substr(0,4)=='http'" :src="item.minsutupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.minsutupian?item.minsutupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","borderRadius":"10px","display":"block","height":"150px"}' v-else :src="baseUrl + (item.minsutupian?item.minsutupian.split(',')[0]:'')" class="image" />
			</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/minsuleixing/minsuleixing').then(res => {
			if (res.data.code == 0) {
				this.minsuleixingOptions = res.data.data;
				this.selectOptionsList.push({name:'民宿类型',list:this.minsuleixingOptions,tableName: 'minsuleixing',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getFenlei() {
		...
		this.feileiColumn = 'minsuleixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.minsumingcheng != '') searchWhere.minsumingcheng = '%' + this.formSearch.minsumingcheng + '%';
        if (this.formSearch.minsuleixing != '') searchWhere.minsuleixing = this.formSearch.minsuleixing;
        if (this.curFenlei != '全部') searchWhere.minsuleixing = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`minsuxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/minsuxinxiDetail', query: params});
      },
